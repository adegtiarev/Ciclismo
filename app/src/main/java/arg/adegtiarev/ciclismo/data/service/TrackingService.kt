...
class TrackingService : LifecycleService() {
    ...
    companion object {
        // Publicly observable state for the UI
        ...
        // Public method for the UI to consume an event
        fun clearEvent() {
            _events.value = null
        }
    }
    ...
    private fun startRide() {
        // If this is a new session (not a resume from pause), reset everything.
        if (!isSessionActive) {
            resetData()
        }
    ...
    private fun stopRide() {
        ...
        serviceScope.launch {
            // Save the ride and get its ID
            saveRideToDb()?.let { rideId ->
                _events.value = "SAVED_$rideId"
            }
            // Stop the service. The data will be reset on the *next* ride start.
    ...
    private fun checkStopDialogConditions(location: Location) {
        ...
        if (hasLeftStartThreshold) {
            if (distanceToStart < 20f && _events.value != TrackingConstants.SHOW_STOP_DIALOG) {
                _events.value = TrackingConstants.SHOW_STOP_DIALOG
            } else if (distanceToStart > 50f && _events.value == TrackingConstants.SHOW_STOP_DIALOG) {
                _events.value = TrackingConstants.HIDE_STOP_DIALOG
            }
        }
    }
    ...
    private fun startTimer() {
        ...
        timerJob = serviceScope.launch {
            while (_isTracking.value) {
                delay(1000L)
                if (_isTracking.value) { // Re-check after delay
